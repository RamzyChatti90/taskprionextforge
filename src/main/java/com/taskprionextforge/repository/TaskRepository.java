...
public interface TaskRepository extends JpaRepository<Task, Long> {
    ...
    @Query(value = "select task from Task task left join fetch task.assignee", countQuery = "select count(task) from Task task")
    Page<Task> findAllWithToOneRelationships(Pageable pageable);

    @Query("select task from Task task left join fetch task.assignee")
    List<Task> findAllWithToOneRelationships();

    @Query("select task from Task task left join fetch task.assignee where task.id =:id")
